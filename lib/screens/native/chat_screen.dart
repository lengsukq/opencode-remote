...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void _handleDelta(Map<String, dynamic> data) {
    final rawProps = data['payload'] is Map
    ...
    if (field == 'text' && delta != null) {
      ...
      _streamState.toolStates[partID] = {
        ...?_streamState.toolStates[partID],
        'status': delta,
      };
    } else if (field == 'state.output' && delta != null) {
      final existing =
          (_streamState.toolStates[partID]?['output'] as String? ?? '') + delta;
      _streamState.toolStates[partID] = {
        ...?_streamState.toolStates[partID],
        'output': existing,
      };
  ...
  void _handlePermission(Map<String, dynamic> data) {
    ...
    AppDialog.showCustom(
      context,
      title: S.permissionRequest,
      showDefaultCancel: false,
      content: Text(
        '${props['message'] ?? 'Allow this operation?'}',
        ...
        TextButton(
          onPressed: () {
            ...
            if (permissionID != null) {
              widget.api.executeCommand(
                widget.session.id,
                command: 'permission',
                arguments: ['deny', permissionID],
              );
            }
          },
          child: const Text(S.deny, style: TextStyle(color: AppColors.danger)),
        ),
        TextButton(
          onPressed: () {
            Navigator.pop(context);
            if (permissionID != null) {
              widget.api.executeCommand(
                widget.session.id,
                command: 'permission',
                arguments: ['approve', permissionID, '--remember'],
              );
            }
          },
          child: const Text(
          ...
          onPressed: () {
            ...
            if (permissionID != null) {
              widget.api.executeCommand(
                widget.session.id,
                command: 'permission',
                arguments: ['approve', permissionID],
              );
            }
          },
          child: const Text(S.allowOnce),
        ),
      ],
    );
  }
  ...
  Future<_LoadData> _loadAllData() async {
    ...
    final defaults = (rawDefaults is Map)
        ? (rawDefaults as Map<String, dynamic>).map(
  ...
  void _applyLoadedData(_LoadData data) {
    // 优先从服务器默认配置获取模型，其次从历史消息恢复
    final serverDefault = data.defaults['build'] ?? data.defaults['chat'];
    String? autoModel = serverDefault;
    if (autoModel == null && data.msgs.isNotEmpty) {
      final last = data.msgs.lastWhere(
        (m) => m.model != null,
        orElse: () => data.msgs.last,
      );
    ...
    _scrollToBottom();
    _loadTodos();
    _extractFollowUps(data.msgs);
  }

  void _extractFollowUps(List<Message> msgs) {
    if (msgs.isEmpty) {
      if (_followUps.isNotEmpty) setState(() => _followUps = []);
      return;
    }
    final raw = msgs.last.metadata?['followUp'];
    if (raw is List) {
      final suggestions = raw.whereType<String>().toList();
      if (!listEquals(suggestions, _followUps)) {
        setState(() => _followUps = suggestions);
      }
  ...
  Future<void> _sendShell(String text) async => widget.api.runShell(
    widget.session.id,
    command: text,
  ...
  Future<void> _sendCommand(String text) async {
    ...
    await widget.api.executeCommand(
      widget.session.id,
      command: parts.first,
  ...
  Future<void> _sendText(String text) async {
    ...
    try {
      ...
      debugPrint(
        'ChatScreen._sendMessage: async failed ($e), fallback to sync',
      );
  ...
  Future<void> _pickAttachment() async {
    ...
    try {
      switch (source) {
        case 'image':
          final att = await pickImageAttachment(ImageSource.gallery);
          if (att != null && mounted) setState(() => _attachments.add(att));
        case 'camera':
          final att = await pickImageAttachment(ImageSource.camera);
          if (att != null && mounted) setState(() => _attachments.add(att));
        case 'file':
          final att = await pickFileAttachment();
          if (att != null && mounted) setState(() => _attachments.add(att));
      }
    } catch (e) {
      if (mounted) AppSnackBar.error(context, S.attachmentFailed(e));
    }
  }
  ...
  Future<void> _runShell() async {
    ...
    try {
      ...
      if (mounted) {
        AppSnackBar.error(context, S.shellFailed(e));
        setState(() => _isSending = false);
      }
    }
  }
  ...
  void _applyCode(String code, String? language, BuildContext ctx) async {
    ...
    try {
      ...
      if (mounted) AppSnackBar.error(context, 'Write failed: $e');
    }
  }
  ...
  void _showAgentPicker() async {
    final result = await showAgentPicker(
      context,
      agents: _agents,
  ...
  Future<String?> _showActionSheet(Message msg) {
    final options = <BottomSheetOption<String>>[
      ...
      if (msg.role != 'user') ...[
        const BottomSheetOption(
          icon: Icons.undo,
          label: S.revertMessage,
          value: 'revert',
        ),
        const BottomSheetOption(
          icon: Icons.redo,
          label: S.restoreReverted,
          value: 'unrevert',
        ),
        const BottomSheetOption(
      ...
      const BottomSheetOption(
        ...
        label: S.forkFromHere,
        value: 'fork',
      ),
    ];
    return AppBottomSheet.showOptions(
      context,
      title: S.messageActions,
      options: options,
    );
  }
  ...
  Future<void> _showMessageDetailDialog(Message msg) async {
    try {
      final detail = await widget.api.getMessageDetail(
        widget.session.id,
        msg.id,
      );
  ...
  Future<void> _doFork(Message msg) async {
    try {
      final session = await widget.api.forkSession(
        widget.session.id,
        messageID: msg.id,
      );
      if (mounted) {
    ...
    return CallbackShortcuts(
      bindings: {
        ...
        const SingleActivator(
          ...
          _inputCtrl.clear();
          _inputCtrl.text = '/';
          _inputCtrl.selection = const TextSelection.collapsed(offset: 1);
        },
      },
      child: Focus(
        autofocus: true,
          ...
          body: Column(
            children: [
              ...
              if (_revertSnapshot != null)
                RevertBanner(onRestore: _handleRestore),
              if (_cmdState.show && _cmdState.filtered.isNotEmpty)
                CommandSuggestions(
                  commands: _cmdState.filtered,
              ...
              if (_todos.any((t) => !t.done))
                TodoBanner(
                  done: _todos.where((t) => t.done).length,
                  total: _todos.length,
                ),
              if (_followUps.isNotEmpty)
                FollowUpBar(suggestions: _followUps, onSend: _sendFollowUp),
              ChatInputBar(
  ...
  Widget _buildMessageList() {
    ...
    return ListView.builder(
      ...
      itemBuilder: (ctx, i) {
        ...
        final isSecondLatest =
            !isLatest &&
            (_messages[i].role != 'user') &&
            (i == _messages.length - 2);
        return MessageBubble(
          message: _messages[i],
          isLatest: isLatest || isSecondLatest,
          streamingText:
              _streamState.deltas.isNotEmpty &&
                  isLatest &&
                  !_messages[i].content.endsWith('\n')
              ? _streamState.deltas.values.last
              : null,
  ...
  Color? _resolveAgentColor(String agentName) {
    final colorStr = _agents
        .where((a) => a.name == agentName)
        .firstOrNull
        ?.color;
    if (colorStr != null && colorStr.startsWith('#')) {
      final p = int.tryParse(colorStr.replaceFirst('#', '0xFF'));
