  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          // Simple monospace code display without syntax highlighting dependency
          SelectableText(
            code,
            style: TextStyle(
...
class _ToolPartWidgetState extends State<ToolPartWidget> {
    ...
    return Padding(
      ...
      child: Container(
        decoration: BoxDecoration(
          ...
          border: Border.all(color: tool.isError ? AppColors.danger : AppColors.border),
        ),
  ...
  ({Color color, IconData icon, String label}) _statusInfo(ToolPartData tool) {
    ...
    if (tool.isRunning) {
      return (color: AppColors.warning, icon: Icons.hourglass_top, label: '进行中');
    }
    if (tool.isCompleted) {
      return (color: AppColors.success, icon: Icons.check_circle_outline, label: '完成');
    }
    return (color: AppColors.textSecondary, icon: Icons.schedule, label: '等待');
  }

  Widget _buildHeader(String title, String toolName,
  ...
  List<Widget> _buildDiagnostics(Map<String, dynamic> input) {
    ...
    final items = raw.take(_kMaxDiagnosticCount).whereType<Map<String, dynamic>>().toList();
    ...
    return items.map((d) {
      ...
      return Container(
        ...
        child: Row(
          children: [
            Icon(isError ? Icons.error : Icons.warning_amber, size: 14,
