...
class OpenCodeApi {
  ...
  OpenCodeApi({required this.baseUrl, this.username = 'opencode', this.password = '', this.directory}) {
  ...
  static List<T> safeList<T>(dynamic json, T Function(Map<String, dynamic>) fromJson) {
    ...
    return json.map((e) {
      ...
      try { return fromJson(e); } catch (err) { _log('safeList: $err'); return null; }
    }).whereType<T>().toList();
  }
  ...
  static Map<String, String>? modelRef(String model) {
    ...
    return parts.length == 2 ? {'providerID': parts[0], 'modelID': parts[1]} : null;
  }

  // -- Global / Health --
  Future<HealthStatus> getHealth() async {
    final r = await httpGet('/global/health'); check(r);
    return HealthStatus.fromJson(safeMap(jsonDecode(r.body)));
  }

  // -- Project --
  Future<Project> getCurrentProject() async {
    final r = await httpGet('/project/current'); check(r);
    return Project.fromJson(safeMap(jsonDecode(r.body)));
  }
  Future<List<Project>> getProjects() async {
    final r = await httpGet('/project'); check(r);
    return safeList(jsonDecode(r.body), Project.fromJson);
  }
  Future<Project> addProject(String worktree) async {
    final r = await httpPost('/project', body: {'worktree': worktree}); check(r);
    return Project.fromJson(safeMap(jsonDecode(r.body)));
  }
  Future<void> removeProject(String id) async {
    final r = await httpDelete('/project/$id'); check(r);
  }

  Future<VcsInfo?> getVcs() async {
    final r = await httpGet('/vcs'); check(r);
    return VcsInfo.fromJson(safeMap(jsonDecode(r.body)));
  }
  Future<PathInfo> getPath() async {
    final r = await httpGet('/path'); check(r);
    return PathInfo.fromJson(safeMap(jsonDecode(r.body)));
  }

  // -- Instance --
  Future<void> disposeInstance() async {
    final r = await httpPost('/instance/dispose'); check(r);
  }

  // -- Session CRUD --
  Future<List<Session>> getSessions() async {
    final r = await httpGet('/session'); check(r);
    return safeList(jsonDecode(r.body), Session.fromJson);
  }
  Future<Session> createSession({String? parentID, String? title}) async {
    final b = <String, dynamic>{}; addIfNotNull(b, 'parentID', parentID); addIfNotNull(b, 'title', title);
    final r = await httpPost('/session', body: b); check(r);
    return Session.fromJson(safeMap(jsonDecode(r.body)));
  }
  Future<Session> getSession(String id) async {
    final r = await httpGet('/session/$id'); check(r);
    return Session.fromJson(safeMap(jsonDecode(r.body)));
  }
  Future<void> deleteSession(String id) async { final r = await httpDelete('/session/$id'); check(r); }
  Future<void> abortSession(String id) async { final r = await httpPost('/session/$id/abort'); check(r); }
  Future<Session> updateSession(String id, {String? title}) async {
    final b = <String, dynamic>{}; addIfNotNull(b, 'title', title);
    final r = await httpPatch('/session/$id', body: b); check(r);
    return Session.fromJson(safeMap(jsonDecode(r.body)));
  }

  // -- Session Actions --
  Future<List<Session>> getChildSessions(String id) async {
    final r = await httpGet('/session/$id/children'); check(r);
    return safeList(jsonDecode(r.body), Session.fromJson);
  }
  Future<Session> forkSession(String id, {String? messageID}) async {
    final b = <String, dynamic>{}; addIfNotNull(b, 'messageID', messageID);
    final r = await httpPost('/session/$id/fork', body: b); check(r);
    return Session.fromJson(safeMap(jsonDecode(r.body)));
  }
  Future<Session> shareSession(String id) async {
    final r = await httpPost('/session/$id/share'); check(r);
    return Session.fromJson(safeMap(jsonDecode(r.body)));
  }
  Future<Session> unshareSession(String id) async {
    final r = await httpDelete('/session/$id/share'); check(r);
    return Session.fromJson(safeMap(jsonDecode(r.body)));
  }
  Future<List<DiffEntry>> getSessionDiff(String id, {String? messageID}) async {
    var p = '/session/$id/diff'; if (messageID != null) p += '?messageID=$messageID';
    final r = await httpGet(p); check(r);
  ...
  Future<void> summarizeSession(String id, {String? providerID, String? modelID}) async {
    final b = <String, dynamic>{}; addIfNotNull(b, 'providerID', providerID); addIfNotNull(b, 'modelID', modelID);
    final r = await httpPost('/session/$id/summarize', body: b); check(r);
  }
  Future<void> revertMessage(String id, String messageID, {String? partID}) async {
    final b = <String, dynamic>{'messageID': messageID}; addIfNotNull(b, 'partID', partID);
    final r = await httpPost('/session/$id/revert', body: b); check(r);
  }
  Future<void> unrevertMessages(String id) async { final r = await httpPost('/session/$id/unrevert'); check(r); }
  Future<List<Todo>> getSessionTodo(String id) async {
    final r = await httpGet('/session/$id/todo'); check(r);
  ...
  Future<List<Message>> getMessages(String sessionId, {int? limit}) async {
    ...
    final r = await httpGet('/session/$sessionId/message$q'); check(r);
  ...
  Future<SessionMessageResponse> getMessageDetail(String sessionId, String messageId) async {
    final r = await httpGet('/session/$sessionId/message/$messageId'); check(r);
    return SessionMessageResponse.fromJson(safeMap(jsonDecode(r.body)));
  }
  Future<SessionMessageResponse> sendMessage(String sessionId, {String? content, List<Map<String, dynamic>>? parts,
    String? model, String? agent, String? messageID, bool? noReply, String? system,
    List<Map<String, dynamic>>? tools}) async {
    final bodyParts = parts ?? [{'type': 'text', 'text': content ?? ''}];
    final b = <String, dynamic>{'parts': bodyParts};
    if (model != null) b['model'] = modelRef(model);
    addIfNotNull(b, 'agent', agent); addIfNotNull(b, 'messageID', messageID);
    addIfNotNull(b, 'noReply', noReply); addIfNotNull(b, 'system', system); addIfNotNull(b, 'tools', tools);
    final r = await httpPost('/session/$sessionId/message', body: b); check(r);
    return SessionMessageResponse.fromJson(safeMap(jsonDecode(r.body)));
  }
  Future<void> sendMessageAsync(String sessionId, {String? content, List<Map<String, dynamic>>? parts,
    String? model, String? agent, String? messageID, bool? noReply, String? system,
    List<Map<String, dynamic>>? tools}) async {
    final bodyParts = parts ?? [{'type': 'text', 'text': content ?? ''}];
    final b = <String, dynamic>{'parts': bodyParts};
    ...
    addIfNotNull(b, 'agent', agent); addIfNotNull(b, 'messageID', messageID);
    addIfNotNull(b, 'noReply', noReply); addIfNotNull(b, 'system', system); addIfNotNull(b, 'tools', tools);
    final r = await httpPost('/session/$sessionId/prompt_async', body: b); check(r);
  }

  // -- Terminal / Shell --
  Future<SessionMessageResponse> executeCommand(String sessionId, {required String command,
    required List<String> arguments, String? agent, String? model, String? messageID}) async {
    final b = <String, dynamic>{'command': command, 'arguments': arguments};
    addIfNotNull(b, 'agent', agent); addIfNotNull(b, 'model', model); addIfNotNull(b, 'messageID', messageID);
    final r = await httpPost('/session/$sessionId/command', body: b); check(r);
    return SessionMessageResponse.fromJson(safeMap(jsonDecode(r.body)));
  }
  Future<SessionMessageResponse> runShell(String sessionId, {required String command, String? agent,
    String? model}) async {
    final b = <String, dynamic>{'command': command};
    addIfNotNull(b, 'agent', agent); addIfNotNull(b, 'model', model);
    final r = await httpPost('/session/$sessionId/shell', body: b); check(r);
    return SessionMessageResponse.fromJson(safeMap(jsonDecode(r.body)));
  }

  // -- Commands --
  Future<List<Command>> getCommands() async {
    final r = await httpGet('/command'); check(r);
  ...
  Future<Map<String, SessionStatus>> getSessionStatuses() async {
    final r = await httpGet('/session/status'); check(r);
    return safeMap(jsonDecode(r.body)).map((k, v) => MapEntry(k, SessionStatus.fromJson(safeMap(v))));
  }
  Future<void> initSession(String id, {required String messageID, String? providerID, String? modelID}) async {
    ...
    addIfNotNull(b, 'providerID', providerID); addIfNotNull(b, 'modelID', modelID);
    final r = await httpPost('/session/$id/init', body: b); check(r);
  }
  Future<void> respondPermission(String sessionId, String permissionID, {required String response,
    bool? remember}) async {
    final b = <String, dynamic>{'response': response}; if (remember != null) b['remember'] = remember;
    final r = await httpPost('/session/$sessionId/permissions/$permissionID', body: b); check(r);
  }

  // -- Files --
  Future<List<FileNode>> listFiles(String path) async {
    final r = await httpGet('/file/browse?path=$path'); check(r);
    return safeList(jsonDecode(r.body), FileNode.fromJson);
  }
  Future<FileContent> readFile(String path) async {
    final r = await httpGet('/file/read?path=$path'); check(r);
    return FileContent.fromJson(safeMap(jsonDecode(r.body)));
  }
  Future<List<String>> findFiles(String query) async {
    final r = await httpGet('/file/find?pattern=$query'); check(r);
    final data = jsonDecode(r.body);
    return data is List ? data.whereType<String>().toList() : [];
  }
  Future<List<SearchMatch>> searchFiles(String query) async {
    final r = await httpGet('/file/search?query=$query'); check(r);
    return safeList(jsonDecode(r.body), SearchMatch.fromJson);
  }
  Future<List<Symbol>> findSymbols(String query) async {
    final r = await httpGet('/file/symbol?query=$query'); check(r);
    return safeList(jsonDecode(r.body), Symbol.fromJson);
  }
  Future<List<FileStatus>> getFileStatus() async {
    final r = await httpGet('/file/status'); check(r);
    return safeList(jsonDecode(r.body), FileStatus.fromJson);
  }

  // -- Config --
  Future<Config> getConfig() async {
    final r = await httpGet('/config'); check(r);
    return Config.fromJson(safeMap(jsonDecode(r.body)));
  }
  Future<Config> patchConfig(Map<String, dynamic> updates) async {
    final r = await httpPatch('/config', body: updates); check(r);
    return Config.fromJson(safeMap(jsonDecode(r.body)));
  }
  Future<Map<String, dynamic>> getConfigProviders() async {
    final r = await httpGet('/config/providers'); check(r);
  ...
  Future<List<Provider>> getProviders() async {
    final r = await httpGet('/provider'); check(r);
    final data = jsonDecode(r.body);
  ...
  Future<Map<String, List<ProviderAuthMethod>>> getProviderAuth() async {
    final r = await httpGet('/provider/auth'); check(r);
    return safeMap(jsonDecode(r.body)).map((k, v) => MapEntry(k, safeList(v, ProviderAuthMethod.fromJson)));
  }
  Future<void> setAuth(String id, Map<String, dynamic> body) async {
    final r = await httpPut('/auth/$id', body: body); check(r);
  }

  // -- Agents --
  Future<List<Agent>> getAgents() async {
    final r = await httpGet('/agent'); check(r);
    return safeList(jsonDecode(r.body), Agent.fromJson);
  }

  // -- Extensions (LSP / Formatter / MCP) --
  Future<List<LSPStatus>> getLspStatus() async {
    final r = await httpGet('/lsp'); check(r);
    return safeList(jsonDecode(r.body), LSPStatus.fromJson);
  }
  Future<List<FormatterStatus>> getFormatterStatus() async {
    final r = await httpGet('/formatter'); check(r);
    return safeList(jsonDecode(r.body), FormatterStatus.fromJson);
  }
  Future<Map<String, MCPStatus>> getMcpStatus() async {
    final r = await httpGet('/mcp'); check(r);
    return safeMap(jsonDecode(r.body)).map((k, v) => MapEntry(k, MCPStatus.fromJson(safeMap(v))));
  }

  // -- Log --
  Future<void> writeLog(String service, String level, String message, {Map<String, dynamic>? extra}) async {
    final b = <String, dynamic>{'service': service, 'level': level, 'message': message};
    if (extra != null) b['extra'] = extra;
    final r = await httpPost('/log', body: b); check(r);
  }

  // -- Provider OAuth --
  Future<ProviderAuthAuthorization> oauthAuthorize(String id, {required int method}) async {
    final r = await httpPost('/provider/$id/oauth/authorize', body: {'method': method}); check(r);
    return ProviderAuthAuthorization.fromJson(safeMap(jsonDecode(r.body)));
  }

  // -- Experimental --
  Future<bool> getCurrentModelConfigured({String? providerID, String? modelID}) async {
    final b = <String, dynamic>{}; addIfNotNull(b, 'providerID', providerID); addIfNotNull(b, 'modelID', modelID);
    final r = await httpPost('/model/current/configured', body: b); check(r);
