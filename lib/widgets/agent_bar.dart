...
class AgentBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final totalTokens = tokens != null
        ? tokens!.input + tokens!.output + tokens!.reasoning
        : 0;
    final contextPct = totalTokens > 0
        ? (totalTokens / _kContextWindowTokens).clamp(0.0, 1.0)
        : 0.0;
    final ctxColor = contextPct < 0.5
        ? AppColors.success
        : (contextPct < 0.8 ? AppColors.warning : AppColors.danger);
      ...
      child: Row(
        children: [
          ...
          if (totalTokens > 0) ...[
            ...
            Tooltip(
              message:
                  'Context: $totalTokens / ${_kContextWindowTokens ~/ 1000}K tokens',
              child: Container(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Text(
                      ...
                      style: TextStyle(color: ctxColor, fontSize: 10),
                    ),
                  ],
                ),
              ),
